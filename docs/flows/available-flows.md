...
| [RAG Evaluation](#rag-evaluation-icl-dataset-flow) | 1 | RAG evaluation with realistic questions | `rag-evaluation`, `icl` |
...
**Name:** `RAG Evaluation ICL Dataset Flow`

**Purpose:** Generate realistic Q&A pairs for RAG evaluation using the existing 3-stage question generation pipeline with ICL-driven style evolution.

**Location:** `src/sdg_hub/flows/evaluation/rag_evaluation_icl/`

### Architecture

**3-Stage Pipeline with ICL Style Evolution:**
...
Uses the same 3-stage pipeline as the base RAG evaluation flow, but replaces the evolution step with ICL-driven style transfer:
...
5. Evaluates answer groundedness on a 1-5 scale and filters out poorly grounded pairs
