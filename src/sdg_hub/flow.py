...
# Mapping of string operator names to actual operator functions
...
# Mapping of string data type names to their conversion functions\
...
class Flow(ABC):
    """Abstract base class for managing data processing flows.
    
    This class handles the configuration and execution of data processing pipelines
    defined in YAML files. It manages LLM clients, block registration, and
    configuration file resolution.

    Attributes:
        llm_client: The language model client instance
        num_samples_to_generate (Optional[int]): Number of samples to generate
        base_path (str): Base path for resource files
        registered_blocks (dict): Dictionary of registered block types
    """
    ...
    def get_flow_from_file(self, yaml_path: str) -> "Flow":
        ...
        Load and process a flow configuration from a YAML file.

        This method:
        1. Loads the YAML configuration
        2. Processes LLM blocks by adding the client and prompt template (see detailed implementation in blocks/blocks.py, blocks/llmblocks.py, blocks/utilblocks.py)
        3. Converts block types to their corresponding class instances
        4. Resolves configuration file paths
        5. Processes operation and data type conversion mappings
        ...
        # Process each block in the flow
        for block in flow:
            # Handle LLM blocks
            if "LLM" in block["block_type"]:
                # Add LLM client to block configuration
                block["block_config"]["client"] = self.llm_client
                
                # Handle model prompt template
                if block["block_config"].get("model_prompt", None) is None:
                    # Try to find a matching prompt template in the registry
                    matched_prompt = next(
                        (
                            key
            ...
            # Convert block type to actual class instance
            try:
            ...
            # Handle multiple configuration paths (dict or list)
            ...
            # Convert operation string to actual operator function
            if "operation" in block["block_config"]:
                block["block_config"]["operation"] = OPERATOR_MAP[
                    block["block_config"]["operation"]
                ]

            # Convert data type string to actual conversion function
