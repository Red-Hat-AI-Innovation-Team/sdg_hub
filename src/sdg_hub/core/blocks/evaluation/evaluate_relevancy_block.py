...
class EvaluateRelevancyBlock(BaseBlock):
    ...
    **kwargs : Any
        ...
        - **TextParserBlock parameters**: start_tags (default: ["[Start of Feedback]", "[Start of Score]"]),
          end_tags (default: ["[End of Feedback]", "[End of Score]"]), parsing_pattern,
          parser_cleanup_tags, expand_lists.

        - **ColumnValueFilterBlock parameters**: filter_value (default: 2.0), operation (default: "eq"),
    ...
    def __init__(self, **kwargs):
        ...
        if model:
            logger.info(
                f"Initialized EvaluateRelevancyBlock '{self.block_name}' with model '{model}'",
                extra={
                    "block_name": self.block_name,
                    "model": model,
                    "async_mode": self.llm_params.get("async_mode", True),
                    "filter_value": self.filter_params.get("filter_value", 2.0),
                },
            )
