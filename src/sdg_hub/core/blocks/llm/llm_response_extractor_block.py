...
class LLMResponseExtractorBlock(BaseBlock):
    ...
    def _extract_fields_from_response(self, response: dict) -> dict[str, Any]:
        ...
        dict[str, Any]
            ...
            All requested columns are always present with defaults
            (``""`` for content/reasoning_content, ``[]`` for tool_calls).
        """
        ...
        if self.extract_content:
            ...
            extracted[self._content_field] = content if content is not None else ""
            if content is None:
