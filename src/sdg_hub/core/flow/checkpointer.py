...
class FlowCheckpointer:
    ...
    def load_existing_progress(self, input_dataset: Dataset) -> Tuple[Dataset, Optional[Dataset]]:
        ...
        try:
            ...
            remaining_dataset = self._find_remaining_samples(input_dataset, completed_dataset)
    ...
    def save_final_checkpoint(self) -> None:
        ...
        if self._pending_samples:
            self._save_checkpoint()
            logger.info(f"Saved final checkpoint with {len(self._pending_samples)} samples")
    ...
    def _save_checkpoint(self) -> None:
        ...
        checkpoint_file = os.path.join(
            self.checkpoint_dir, 
            f"checkpoint_{self._checkpoint_counter:04d}.jsonl"
        )
        ...
        for file_path in checkpoint_files:
            try:
                ...
                if len(dataset) > 0:
                    ...
                    logger.debug(f"Loaded checkpoint: {file_path} ({len(dataset)} samples)")
            except Exception as exc:
    ...
    def _find_remaining_samples(self, input_dataset: Dataset, completed_dataset: Dataset) -> Dataset:
