...
DATA_PATH = "/Users/ashtarkb/Desktop/sdg_hub_latest/sdg_hub/my_example/ibm_seed_data.jsonl"
...
def main():
    ...
    for block in dry_result_1.get('blocks_executed', []):
        if 'LLM' in block.get('block_type', '') or 'Evaluate' in block.get('block_type', ''):
            print(f"   - {block['block_name']}: {block.get('execution_time_seconds', 0):.2f}s, {block.get('input_rows', 0)} requests")
    print()
    ...
    if prediction['estimated_time_seconds'] < 3600:
        print(f"   {est_mins:.1f} minutes ({prediction['estimated_time_seconds']:.0f} seconds)")
    else:
    ...
    if prediction.get('total_estimated_requests'):
        ...
        print(f"   Requests per sample: {prediction['total_estimated_requests']/200:.1f}")
    ...
    print(f"   • This will make ~{prediction.get('total_estimated_requests', 0):,} LLM requests")
    print(f"   • Using max_concurrency=100")
