...
- role: user
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      actions={[
        <Button key="save" variant="primary" onClick={handleSave} isLoading={isSaving}>
          Continue
        </Button>,
    ...
    >
      <Form>
        {/* File Upload */}
        <FormGroup fieldId="prompt-file-upload" style={{ marginBottom: '1rem' }}>
          <FileUpload
            id="prompt-file-upload"
            type="text"
            value=""
            filename=""
            filenamePlaceholder="Upload a YAML prompt file to auto-fill..."
            onFileInputChange={handleFileUpload}
            hideDefaultPreview
            browseButtonText="Upload Prompt"
          />
        </FormGroup>
        ...
        <div style={{ 
          ...
          <span>
            💡 Use template variables like <code>{'{{document}}'}</code> to insert dynamic content
          </span>
          <Popover
            headerContent={
              <div>
        ...
        >
          <TextInput
            ...
            placeholder="Enter a unique prompt name (e.g., my_qa_prompt)"
          />
        </FormGroup>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
          ...
          >
            <CodeBlock>
              <CodeBlockCode style={{ fontSize: '0.75rem' }}>
{`- role: system
  content: ${systemMessage || '...'}
