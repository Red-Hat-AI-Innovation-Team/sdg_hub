...
def mock_sdg_hub(mock_flow):
    ...
    with patch.dict("sys.modules", {
        ...
        with patch("api_server.Flow", return_value=mock_flow) as mock_flow_cls, \
             patch("api_server.FlowRegistry", MockFlowRegistry) as mock_registry, \
             patch("api_server.BlockRegistry", MockBlockRegistry) as mock_block_registry:
            ...
            yield {
                "Flow": mock_flow_cls,
                "FlowRegistry": mock_registry,
                "BlockRegistry": mock_block_registry,
            }
...
def test_client(mock_sdg_hub, temp_dir) -> TestClient:
    ...
    with patch("api_server.UPLOADS_DIR", Path(temp_dir) / "uploads"), \
         patch("api_server.CUSTOM_FLOWS_DIR", Path(temp_dir) / "custom_flows"), \
         patch("api_server.CHECKPOINTS_DIR", Path(temp_dir) / "checkpoints"), \
         patch("api_server.SAVED_CONFIG_FILE", Path(temp_dir) / "saved_configurations.json"), \
         patch("api_server.RUNS_HISTORY_FILE", Path(temp_dir) / "runs_history.json"):
        
        # Create directories
        (Path(temp_dir) / "uploads").mkdir(exist_ok=True)
        (Path(temp_dir) / "custom_flows").mkdir(exist_ok=True)
        (Path(temp_dir) / "checkpoints").mkdir(exist_ok=True)
