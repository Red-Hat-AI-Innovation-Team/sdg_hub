...
from sdg_hub import FlowRegistry, Flow, BlockRegistry
...
if extra_dirs:
    for raw_dir in extra_dirs.split(os.pathsep):
        ...
        try:
            ...
            logger.warning(f"⚠️ Could not prepare dataset directory '{candidate}': {exc}")
            continue
    ...
    for allowed_dir in ALLOWED_DATASET_DIRS:
        if resolved == allowed_dir or allowed_dir in resolved.parents:
            if not resolved.exists():
                raise HTTPException(
                    status_code=404,
                    detail=f"Dataset file not found: {path_str}"
                )
            return resolved
    allowed_text = ", ".join(str(d) for d in ALLOWED_DATASET_DIRS)
    raise HTTPException(
        status_code=400,
        detail=f"Datasets must reside within: {allowed_text}"
    )
...
def sanitize_model_config(config: Dict[str, Any], mask_key: bool = True) -> Dict[str, Any]:
    """
    ...
    if 'api_key' in sanitized:
        ...
        is_safe_value = (
            api_key in ["EMPTY", "NONE", ""] or  # Special test values
            api_key.startswith("env:")  # Environment variable references
        )
...
def validate_api_key_format(api_key: str, provider: Optional[str] = None) -> tuple[bool, Optional[str]]:
    """
...
def generation_worker(log_queue, flow_path, model_config, dataset_params, max_concurrency, log_dir, checkpoint_dir=None, save_freq=None, resume_from_checkpoint=False):
    ...
    try:
        ...
        if dataset_params:
            ...
            if file_format == 'auto':
                ...
                format_map = {'.jsonl': 'jsonl', '.json': 'json', '.csv': 'csv', '.parquet': 'parquet', '.pq': 'parquet'}
            ...
            if file_format == 'jsonl':
                ...
                df = pd.read_csv(data_files, delimiter=csv_delimiter, encoding=csv_encoding)
            ...
            if dataset_params.get('shuffle'):
                df = df.sample(frac=1, random_state=dataset_params.get('seed', 42)).reset_index(drop=True)
            
            # Limit samples if specified
            if dataset_params.get('num_samples'):
                df = df.head(min(dataset_params['num_samples'], len(df)))
        else:
            log_queue.put({'type': 'error', 'message': 'No dataset parameters provided'})
            return
                ...
                if flow_dir.is_dir():
                    ...
                    if flow_yaml.exists():
                        try:
                            ...
                            logger.warning(f"Could not load custom flow from {flow_dir}: {e}")
...
async def search_flows(request: FlowSearchRequest):
    ...
    try:
        if request.tag:
            ...
            flow_names = [f for f in all_flow_names if request.name_filter.lower() in f.lower()]
            logger.info(f"Found {len(flow_names)} flows matching '{request.name_filter}'")
        else:
            flows = FlowRegistry.list_flows()
            flow_names = [flow["name"] for flow in flows]
...
async def get_flow_info(flow_name: str):
    ...
    try:
        ...
        actual_flow_name = flow_name.replace(" (Custom)", "") if is_custom else flow_name
...
async def configure_model(config: ModelConfigRequest):
    ...
    try:
        ...
        safe_config = sanitize_model_config(current_config["model_config"], mask_key=True)
...
async def upload_dataset_file(request: Request):
    ...
    try:
        ...
        logger.info(f"📊 Uploaded dataset file: {file_path} ({bytes_written} bytes, format: {file_extension})")
...
async def load_dataset_from_file(request: DatasetLoadRequest):
    ...
    try:
        ...
        logger.info(f"📊 Loaded dataset from {safe_dataset_path}: {len(df)} samples, {len(df.columns)} columns (pandas DataFrame)")
    ...
    async def generate_logs():
        ...
        try:
            ...
            if config_id and config_id in saved_configurations:
                ...
                logger.info(f"🔧 Loading isolated config for generation: {config_id} (flow: {config.flow_name})")
                ...
                if dataset_config.get('data_files') and dataset_config.get('data_files') != '.':
                    # Construct dataset_load_params from saved dataset_configuration
                    safe_dataset_path = resolve_dataset_file(dataset_config['data_files'])
                    dataset_params = {
                        'data_files': str(safe_dataset_path),
                ...
                flow_name = config.flow_name or (flow_obj.metadata.name if flow_obj else "unknown")
            ...
            if active_generation_process.get("pid") and active_generation_process.get("config_id") == config_id:
                old_pid = active_generation_process["pid"]
                logger.warning(f"⚠️ Previous generation for config {config_id} still active (PID={old_pid}). Killing it.")
                try:
                    import os
                    import signal
            ...
            process = ctx.Process(
                ...
                args=(log_queue, flow_path, model_config, dataset_params, max_concurrency, log_dir, checkpoint_dir, save_freq if enable_checkpoints else None, resume_from_checkpoint)
            )
            ...
            logger.info(f"🚀 Generation worker started (PID={process.pid}) for flow: {flow_path} (config_id={config_id})")
            ...
            process.join()
            logger.info(f"🛑 Generation worker finished (PID={process.pid}, exit_code={process.exitcode}, config_id={config_id})")
            active_generation_process["pid"] = None
            active_generation_process["config_id"] = None
            ...
            if process.exitcode != 0:
                ...
                msg = "Generation cancelled." if process.exitcode in [-15, -9] else f"Process exited with code {process.exitcode}"
...
async def reconnect_stream(config_id: str):
    ...
    async def generate_logs():
        ...
        try:
            ...
            process.join()
            logger.info(f"🛑 Reconnected generation finished (PID={process.pid}, config_id={config_id})")
...
async def cancel_generation(config_id: Optional[str] = None):
    ...
    try:
        ...
        logger.info(f"🧵 Cancel request received. Active PID: {pid}, active_config_id: {active_config_id}, requested_config_id: {config_id}")
        
        if config_id and active_config_id and config_id != active_config_id:
            logger.warning(f"⚠️ Cancel request for config {config_id} ignored; active config is {active_config_id}.")
            return {
                "status": "ignored",
        ...
        if pid:
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail=f"Failed to cancel process {pid}: {kill_error}")
        else:
            logger.info("ℹ️ Cancel requested but no active generation process was tracked.")
            active_generation_process["config_id"] = None
            # Clean up from active_generations too
            if config_id and config_id in active_generations:
                del active_generations[config_id]
...
async def delete_checkpoints(config_id: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to clear checkpoints for {config_id}"
            )
    except HTTPException:
        raise
    except Exception as e:
...
async def download_generated():
    ...
    try:
        if not current_config.get("generated_dataset"):
            raise HTTPException(status_code=404, detail="No generated dataset available")
...
async def import_config(file: UploadFile = File(...)):
    ...
    try:
        ...
        if not flow_info or not flow_info.get("name"):
            raise HTTPException(status_code=400, detail="Invalid configuration file: missing flow information")
        ...
        if dataset_cfg and dataset_cfg.get("data_files"):
            try:
                ...
                df = load_dataset_as_pandas(
                    Path(data_files),
                    DatasetFormat(file_format) if file_format != "auto" else DatasetFormat.AUTO,
                    dataset_cfg.get("csv_delimiter", ","),
                    dataset_cfg.get("csv_encoding", "utf-8")
                )
                
                if dataset_cfg.get("shuffle"):
                    df = df.sample(frac=1, random_state=dataset_cfg.get("seed", 42)).reset_index(drop=True)
...
async def get_current_config():
    ...
    safe_model_config = sanitize_model_config(current_config["model_config"], mask_key=True)
...
async def save_custom_flow(flow_data: Dict[str, Any]):
    ...
    try:
        ...
        flow_dir = ensure_within_directory(CUSTOM_FLOWS_DIR, CUSTOM_FLOWS_DIR / safe_name)
        logger.info(f"Flow directory: {flow_dir} (original name: {flow_name}, base name: {base_flow_name})")
        ...
        if temp_flow_name:
            ...
            temp_flow_dir = ensure_within_directory(CUSTOM_FLOWS_DIR, CUSTOM_FLOWS_DIR / temp_slug)
        ...
        if source_flow_name:
            try:
                ...
                if source_flow_path:
                    ...
                    logger.info(f"Source flow directory from name '{source_flow_name}': {source_flow_dir}")
            except Exception as e:
        ...
        blocks = flow_data.get('blocks', [])
        for block in blocks:
            if 'block_config' in block and 'prompt_config_path' in block['block_config']:
                old_path = block['block_config']['prompt_config_path']
                
                # Extract just the filename from the old path
                old_path_obj = Path(old_path)
                prompt_filename = sanitize_filename(
                    old_path_obj.name
                ...
                new_prompt_file = ensure_within_directory(flow_dir, flow_dir / prompt_filename)
                ...
                if block_name and block_name in wizard_prompts:
                    ...
                    with open(new_prompt_file, 'w') as f:
                        yaml.dump(wizard_prompts[block_name], f, default_flow_style=False)
                    ...
                    if new_prompt_file.exists():
                        logger.info(f"✓ Prompt already exists in flow directory, skipping copy: {new_prompt_file}")
                    ...
                    if not source_file and source_flow_dir:
                        ...
                        if source_in_flow_dir.exists():
                            ...
                            logger.info(f"✓ Found prompt in source flow (full path): {source_file}")
                        else:
                            # Try with just the filename
                            source_in_flow_dir = (source_flow_dir / prompt_filename).resolve()
                            if source_in_flow_dir.exists():
                                source_file = source_in_flow_dir
                                logger.info(f"✓ Found prompt in source flow (filename): {source_file}")
                    
                    if not source_file:
                        logger.warning(f"Prompt source not found for block {block_name} ({old_path}).")
                    ...
                    if source_file:
                        ...
                        allowed_prompt_dirs = [d for d in [flow_dir, temp_flow_dir] if d]
                        ...
                        if source_flow_dir:
                            ...
                            while current_dir and current_dir.name:
                                ...
                                if current_dir.name in ('flows', 'sdg_hub', 'custom_flows'):
                                    break
                                current_dir = current_dir.parent
                        
                        # Check if source file is within any of the allowed directories
                        is_allowed = any(
                            resolved_source == d or d in resolved_source.parents 
                            for d in allowed_prompt_dirs
                        )
                        ...
                        logger.info(f"✅ Copied prompt file: {resolved_source} -> {new_prompt_file}")
                    else:
...
async def save_prompt_template(prompt_data: Dict[str, Any]):
    ...
    try:
        ...
        flow_dir = ensure_within_directory(CUSTOM_FLOWS_DIR, CUSTOM_FLOWS_DIR / safe_flow_name)
        flow_dir.mkdir(parents=True, exist_ok=True)
        
        # Save prompt file
        prompt_filename = sanitize_filename(f"{prompt_name}.yaml") or f"prompt_{int(time.time())}.yaml"
...
def load_saved_configurations_from_disk():
    ...
    try:
        ...
        logger.info(f"Loaded {len(saved_configurations)} saved configurations from disk.")
    except Exception as exc:
...
async def save_configuration(request: SaveConfigurationRequest):
    ...
    try:
        ...
        is_direct_api_key = bool(original_api_key) and not (
            original_api_key in ["EMPTY", "NONE", ""] or original_api_key.startswith("env:")
        )
        ...
        response_config["model_configuration"] = sanitize_model_config(
            response_config["model_configuration"], 
            mask_key=True
        )
    ...
    try:
        ...
        for config in saved_configurations.values():
            ...
            config_dict["model_configuration"] = sanitize_model_config(
                config_dict["model_configuration"], 
                mask_key=True
            )
