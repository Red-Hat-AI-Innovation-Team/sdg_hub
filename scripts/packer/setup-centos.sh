...
function configure_dnf {
  ...
  # dnf config-manager --add-repo https://developer.download.nvidia.com/compute/cuda/repos/rhel${OS_VERSION_MAJOR}/${CUDA_REPO_ARCH}/cuda-rhel${OS_VERSION_MAJOR}.repo \u
}

function install_userland_packages {
  # CI tests in GH Actions will require these packages:
  dnf -y install nvtop podman skopeo git-core python3.12 python3.12-devel
}
...
function install_kernel_driver {
  ...
  dnf -y install kernel-devel-$(uname -r)  # also build for the currently-running kernel.
  ...
  dnf -y module install nvidia-driver:${DRIVER_STREAM}-dkms
}
...
function test_kernel_driver {
  ...
  dkms status
  # Sanity-check that the running kernel has loaded nvidia.ko:
  modprobe nvidia
  # Sanity-check that the driver is working (nvidia-smi works):
  nvidia-smi
}
