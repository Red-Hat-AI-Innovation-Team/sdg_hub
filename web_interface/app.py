...
def get_block_types_from_registry():
    ...
    for block_name, block_class in registry.items():
        ...
        config_schema = {"block_name": {"type": "string", "required": True}}
        ...
        for param_name, param in signature.parameters.items():
            ...
            if get_origin(param_type) is list:
                ...
                schema = {"type": schema_type, "required": is_required}
        ...
        block_types[block_name] = {"name": block_name, "config": config_schema}
    return block_types
    ...
    try:
        ...
        for block in flow_data["blocks"]:
            ...
            yaml_block = {
                ...
                "block_config": {
                    "block_name": block_config.get(
                        "block_name", f"{block_type}_{len(yaml_blocks)}"
                    ),
...
def parse_yaml():
    ...
    try:
        ...
        if not isinstance(yaml_blocks, list):
            return jsonify(
                {"error": "Invalid YAML format. Expected a list of blocks."}
            ), 400
